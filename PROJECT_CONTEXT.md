...
- Contact form: email via Resend, reCAPTCHA v3, honeypot, rate limit, stored
  privately in Sanity
- Per-page SEO metadata, sitemap, robots, Open Graph image, Vercel Web Analytics
...
scripts/                        seed-content.ts, seed-bulletins.ts, upload-hero-video.ts, migrate-mission-statement.ts
...
| `NEXT_PUBLIC_SITE_URL` | Canonical/OG base URL | `https://esfworld.us` |
...
- Nav dropdown children are maintained by hand. `navItem.childSource` exists
  ...
  the nav manually.
- `robots.ts` disallows `/studio` but not `/internal` once indexing is on.
  Both require login, so nothing is exposed, but add `/internal` before
  enabling indexing.

**Next steps**
- Connect `esfworld.us`: Vercel → Domains → add it → set the DNS records at
  GoDaddy → add the domain to Sanity CORS origins → set
